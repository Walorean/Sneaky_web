...
@section('admin_content')
    <div class="admin_body">
        <h1><u>ADMIN PANEL - EDIT PRODUCT</u></h1>

        <div class="admin_form_container">
            <form class="form_sections" method="POST" action="{{ route('admin.product.update', $product->product_code) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form_section">
                    ...
                    <div class="form_field">
                        ...
                        <small>Product code cannot be changed.</small>
                    </div>
                    <div class="form_field">
                        ...
                        <select class="categories-select" name="categories[]" multiple size="4">
                            @foreach($categories as $cat)
                                <option value="{{ $cat->category_id }}"
                                    {{ $product->category->contains('category_id', $cat->category_id) ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="brand-select" name="brand">
                            ...
                            @foreach($brands as $brand)
                                <option value="{{ $brand->brand_id }}"
                                    {{ $product->brand == $brand->brand_id ? 'selected' : '' }}>
                                    {{ $brand->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form_field">
                        ...
                        <textarea name="basic_info">{{ old('basic_info', $product->basic_info) }}</textarea>
                    </div>
                    <div class="form_field">
                        <label>Product Origin Information</label>
                        <input type="text" name="origin" value="{{ old('origin', $product->origin) }}"
                               placeholder="e.g., EuroFleet Innovations, Titanium Plaza...">
                    </div>
                </div>
                <div class="form_section">
                    ...
                    <div class="form_field">
                        ...
                        <input type="text" name="material" value="{{ old('material', $product->material) }}"
                               placeholder="Basic information about material...">
                    </div>
                </div>
                <div class="form_section">
                    ...
                    <div class="form_field">
                        ...
                        <input type="number" name="price" value="{{ old('price', $product->price) }}"
                               placeholder="e.g., 129.99" min="0" step="0.01">
                    </div>
                    <div class="form_field">
                        <label>Product Variants</label>
                        <div id="variants_container">
                            @foreach($product->shoes as $i => $shoe)
                                <div class="variant_row">
                                    ...
                                    <div class="variant_body">
                                        ...
                                        <select name="variants[{{ $i }}][color_id]">
                                            ...
                                            @foreach($colors as $cl)
                                                <option value="{{ $cl->color_id }}"
                                                    {{ $shoe->color_id == $cl->color_id ? 'selected' : '' }}>
                                                    {{ $cl->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="variants[{{ $i }}][size_id]">
                                            ...
                                            @foreach($sizes as $sz)
                                                <option value="{{ $sz->size_id }}"
                                                    {{ $shoe->size_id == $sz->size_id ? 'selected' : '' }}>
                                                    {{ $sz->size }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <label>Stock Quantity *</label>
                                        <input type="number" name="variants[{{ $i }}][stock_quantity]" min="1" value="{{ $shoe->stock_quantity }}">
                                        ...
                                        <div class="variant_images">
                                            ...
                                            <input type="file" class="variant_image_input" name="variants[{{ $i }}][images][]" multiple accept="image/*">
                ...
                <div class="form_buttons">
                    <button type="submit" class="btn_create">Save Changes</button>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const colorsHtml = `@foreach($colors as $cl)
                <option value="{{ $cl->color_id }}">{{ $cl->name }}</option>
            @endforeach`;
            const sizesHtml = `@foreach($sizes as $sz)
                <option value="{{ $sz->size_id }}">{{ $sz->size }}</option>
            @endforeach`;
            ...
            addBtn.addEventListener('click', function () {
                ...
                div.innerHTML = `<div class="variant_header">
                    <h3>Shoe ${index + 1}</h3>
                    <button type="button" class="remove_variant_btn">✕</button>
                </div>
                <div class="variant_body">
            ...
            function renumberVariants() {
                container.querySelectorAll('.variant_row').forEach((v, i) => {
                    v.querySelector('h3').innerText = `Shoe ${i + 1}`;
                });
            }
        });
    </script>
...
@push('scripts')
    <script>
        document.addEventListener('change', function (e) {
            if (!e.target.classList.contains('variant_image_input')) return;
            const files           = e.target.files;
            const variantImages   = e.target.closest('.variant_images');
            const previewContainer = variantImages.querySelector('.image_preview_container');
            const countText       = variantImages.querySelector('.image_count');
